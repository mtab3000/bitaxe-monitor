...
<body>
    <div class="container">
        ...
        <div class="footer">
            Auto-refreshes every 5 seconds | Enhanced variance monitoring with directional tracking
        </div>
    </div>
                ...
                .then(data => {
                    ...
                    data.miners.forEach((miner, index) => {
                        if (miner.status === 'ONLINE') {
                                ...
                                <div class="miner-card">
                                    ...
                                    <div class="variance-section">
                                        ...
                                        <div class="variance-row">
                                            ...
                                            <span>${miner.expected_hashrate_gh.toFixed(0)} GH/s</span>
                                        </div>
                                    </div>
                            ...
                            chartsToCreate.push({ index, miner: miner.miner_name, expected: miner.expected_hashrate_gh });
                        } else {
                            minersHtml += `
                                <div class="miner-card">
                                    <div class="miner-header">
                    ...
                    setTimeout(() => {
                        chartsToCreate.forEach(item => {
                            ...
                            chartTypes.forEach(type => {
                                ...
                                if (!charts[chartKey]) {
                                    createChart(`chart-${item.index}-${type}`, item.miner, type, item.expected);
                                }
                            });
                        });
...
class CompleteDemo:
    def __init__(self):
        """
        Initialize the CompleteDemo instance with a Flask app, demo miner baselines, application start time, and rolling history buffers for variance analysis.
        """
    ...
    def setup_routes(self):
        """
        Register Flask routes for the web dashboard and metrics API.
        
        Defines the root route `'/'` to serve the main monitoring interface and the `'/api/metrics'` route to provide simulated mining fleet metrics as a JSON response, including real-time and historical variance statistics for each miner.
        """
        @self.app.route('/')
        def index():
            """
            Serves the main dashboard page for the mining fleet monitoring application.
            
            Returns:
                Rendered HTML page displaying real-time mining statistics and charts.
            """
        ...
        def api_metrics():
            """
            Simulate and return current mining fleet metrics and per-miner statistics as a JSON response.
            
            The response includes real-time hashrate, expected hashrate, efficiency percentage, power consumption, temperature, frequency, uptime, and multi-window hashrate standard deviations for each miner. Aggregated fleet-wide totals and efficiency are also provided.
            """
            miners = []
            total_hashrate = 0
            total_power = 0
            ...
            for name, base_rate in self.base_hashrates.items():
                ...
                power = random.uniform(13.5, 16.2)
                ...
                miner = {
                    ...
                    'power_w': power,
                    'temperature_c': random.uniform(58, 78),
                    'frequency_mhz': random.randint(485, 515),
    ...
    def run(self):
        """
        Launches the enhanced variance monitoring web application and serves the real-time dashboard on port 8080.
        """
        print("=" * 80)
        ...
        print("[OK] Efficiency and standard deviation charts")
        print("[OK] All enhanced variance features working")
        print("=" * 80)
        print()
        print("Web interface: http://localhost:8080")
        print("Features:")
        print("  - Real-time hashrate charts")
        print("  - Directional variance charts (gray baseline + actual)")
        print("  - Efficiency tracking charts")
        print("  - Variance standard deviation charts")
