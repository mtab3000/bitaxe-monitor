...
class RepositoryChecker:
    ...
    def __init__(self, repo_path: str = "."):
        """
        Initialize a RepositoryChecker instance for validating repository consistency.
    ...
    def log_error(self, message: str):
        """
        Log an error message, append it to the internal error list, and print it with an "[ERROR]" tag.
        """
    ...
    def log_warning(self, message: str):
        """
        Log a warning message and add it to the internal warnings list.
        """
    ...
    def log_info(self, message: str):
        """
        Log an informational message and add it to the internal info list.
        """
    ...
    def log_success(self, message: str):
        """
        Prints a success message prefixed with an "[OK]" tag.
        """
        print(f"[OK] {message}")
    
    def check_file_exists(self, file_path: str) -> bool:
        """
        Check if a specified file exists in the repository.
    ...
    def check_ip_consistency(self):
        """
        Check key repository files for the presence of expected miner IP addresses to validate network configuration consistency.
        
        Logs a success if at least two expected IP addresses are found in each file, a warning if fewer are found or if the file is missing, and an error if a file cannot be read.
        """
    ...
    def check_enhanced_monitor_config(self):
        """
        Checks that `enhanced_bitaxe_monitor.py` exists, can be imported, and its main components initialize correctly.
        
        Attempts to import `EnhancedBitAxeMonitor` and `MinerConfig`, instantiate the monitor with a test configuration, and verify that required attributes such as the Flask app and variance trackers are present. Logs errors if the file is missing, imports fail, or required attributes are not found.
        """
    ...
    def check_docker_configuration(self):
        """
        Check Docker configuration files for correct references and network setup.
        
        Verifies that the Dockerfile exists, references the enhanced monitor script, and exposes port 8080. Also checks that docker-compose.yml exists and includes at least two expected miner IP addresses. Logs successes or warnings based on the findings.
        """
    ...
    def check_github_workflows(self):
        """
        Checks for the existence and configuration of GitHub workflow files in the repository.
        
        Verifies that the `.github/workflows` directory exists and contains workflow YAML files. Specifically checks for the `enhanced-quality.yml` workflow and ensures it references `enhanced_bitaxe_monitor.py`, logging warnings if workflows are missing, misconfigured, or have encoding issues.
        """
    ...
    def check_documentation_consistency(self):
        """
        Checks documentation files for references to enhanced features or Chart.js.
        
        Logs a success if enhanced features or Chart.js are mentioned, informational messages if enhancements may be missing, and warnings if documentation files are absent.
        """
    ...
    def check_test_configuration(self):
        """
        Check for the existence and syntax validity of test files and test configuration files in the repository.
        
        Verifies that the `tests` directory exists, ensures the main test file (`test_bitaxe_monitor.py`) is present and free of syntax errors, and logs the presence or absence of optional test configuration files (`test_config.py`, `test_enhanced_monitor.py`).
        """
    ...
    def check_dependencies(self):
        """
        Checks that essential dependencies are listed in requirements files.
        
        Scans 'requirements.txt' and 'requirements-dev.txt' for the presence of 'flask' and 'requests', logging a success if both are found or a warning if either is missing or if the file does not exist.
        """
    ...
    def check_security_and_quality(self):
        """
        Checks for the presence of security and code quality configuration files and validates essential exclusions in `.gitignore`.
        
        Verifies that `.gitignore`, `SECURITY.md`, and `.pylintrc` exist in the repository, and ensures `.gitignore` excludes `__pycache__` and `.env`.
        """
    ...
    def generate_report(self):
        """
        Generate and print a summary report of all repository consistency checks.
        
        Returns:
            bool: True if no errors were found (warnings allowed), False if any errors are present.
        """
        print("\n" + "=" * 60)
    ...
    def run_full_check(self):
        """
        Runs all repository validation checks and generates a summary report.
        
        Executes essential file presence checks, IP address consistency, configuration validation, Docker and GitHub workflow verification, documentation and test inspection, dependency analysis, and security/quality file checks. Returns True if no errors are detected; otherwise, returns False.
        """
...
def main():
    """
    Run all repository consistency checks and print a summary status.
    
    Returns:
        int: 0 if all checks pass without errors, 1 if any issues are detected.
    """
