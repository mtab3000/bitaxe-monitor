...
class BitAxeAPI:
    ...
    def __init__(self, timeout=5):
        """
        Initialize the BitAxeAPI client with an optional HTTP request timeout.
        
        Parameters:
            timeout (int, optional): Timeout in seconds for HTTP requests to miners. Defaults to 5.
        """
        self.timeout = timeout
    
    def get_system_info(self, miner_config: MinerConfig) -> Optional[Dict]:
        """
        Fetches system information from a BitAxe miner's HTTP API.
        
        Attempts to retrieve and parse JSON system metrics from the specified miner. Returns the parsed data if successful, or None if the miner is unreachable or responds with a non-200 status code.
        
        Returns:
            dict or None: Parsed system information if successful; otherwise, None.
        """
        try:
...
class VarianceTracker:
    ...
    def __init__(self, maxlen=600):  # 10 minutes of data at 1-second intervals
        """
        Initialize a VarianceTracker for storing timestamped hashrate data points used in variance calculations.
        
        Parameters:
            maxlen (int): The maximum number of data points to retain, typically representing the number of seconds to track.
        """
    ...
    def add_data_point(self, value: float):
        """
        Add a new hashrate value with the current timestamp to the variance tracker.
        """
    ...
    def get_variance_stats(self, window_seconds: int) -> Optional[float]:
        """
        Compute the standard deviation of data points within the specified recent time window.
        
        Parameters:
            window_seconds (int): Duration in seconds to include data points for the calculation.
        
        Returns:
            float or None: Standard deviation of values in the window, or None if fewer than two data points are present.
        """
        if len(self.data) < 2:
            return None
...
class EnhancedBitAxeMonitor:
    ...
    def __init__(self, miners_config: List[Dict], port=8080):
        """
        Initialize the EnhancedBitAxeMonitor with miner configurations and web server settings.
        
        Creates MinerConfig objects for each miner, sets up variance trackers, and prepares Flask routes for the monitoring dashboard.
        """
    ...
    def setup_routes(self):
        """
        Configures Flask routes for the web dashboard and the API endpoint that returns real-time miner metrics in JSON format.
        """
        
        @self.app.route('/')
        def index():
            """
            Serves the main dashboard page displaying real-time monitoring and charts for all configured BitAxe miners.
            
            Returns:
                str: Rendered HTML page with embedded JavaScript and Chart.js visualizations for live miner statistics.
            """
        ...
        def api_metrics():
            """
            Handles the '/api/metrics' route and returns JSON metrics for all monitored miners, including fleet-wide and per-miner statistics.
            """
            return jsonify(self.get_all_metrics())
    
    def get_miner_metrics(self, miner: MinerConfig) -> MinerMetrics:
        """
        Retrieve current metrics and variance statistics for a single BitAxe miner.
        
        If the miner is unreachable, returns metrics with status 'OFFLINE'. Otherwise, collects system information, calculates efficiency relative to the expected hashrate, updates variance tracking, and computes standard deviations over 60s, 300s, and 600s intervals.
        
        Parameters:
            miner (MinerConfig): Miner configuration specifying the target miner.
        
        Returns:
            MinerMetrics: Real-time metrics and variance statistics for the specified miner.
        """
    ...
    def get_all_metrics(self) -> Dict:
        """
        Aggregate and return current metrics for all configured miners, including fleet-wide statistics.
        
        Returns:
            dict: Contains the current timestamp, total hashrate (TH/s), total power consumption (W), fleet efficiency percentage, number of online miners, total miner count, and a list of per-miner metrics.
        """
    ...
    def run(self):
        """
        Launches the enhanced BitAxe monitor, printing startup details and starting the Flask web server for real-time monitoring and visualization of configured miners.
        """
        print("=" * 80)
...
def main():
    """
    Initializes miner configurations and launches the enhanced BitAxe monitoring web server.
    
    Edit the `miners_config` list to specify your miners' details before running. Sets up logging, creates the monitor instance, and starts the Flask-based dashboard on port 8080.
    """
