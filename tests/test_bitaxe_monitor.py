...
class TestEnhancedMonitor(unittest.TestCase):
    ...
    def test_bitaxe_api_timeout(self):
        """
        Verify that BitAxeAPI.get_system_info returns None when the miner is unreachable.
        """
    ...
    def test_bitaxe_api_success(self, mock_get):
        """
        Verify that BitAxeAPI.get_system_info returns correct system information when the API responds with valid data.
        
        Mocks a successful API response and checks that the returned dictionary includes the expected hashrate and power values.
        """
    ...
    def test_monitor_routes_exist(self):
        """
        Test that the main (`/`) and API (`/api/metrics`) routes of the EnhancedBitAxeMonitor Flask app are accessible and return the expected HTTP status codes and response structure.
        """
        monitor = EnhancedBitAxeMonitor(self.test_config, port=8082)
    ...
    def test_offline_miner_handling(self, mock_api):
        """
        Verify that the monitor sets a miner's status to 'OFFLINE' and retains correct name and IP when the API returns no data.
        """
    ...
    def test_online_miner_metrics(self, mock_api):
        """
        Verify that metrics for an online miner are accurately computed and populated when the API returns valid system information.
        """
        # Mock successful API response
...
class TestClassicMonitor(unittest.TestCase):
    ...
    def test_asic_specs_creation(self):
        """
        Test retrieval of ASIC specifications for the 'BM1370' model and verify that required fields are present.
        """
...
class TestConfigurationValidation(unittest.TestCase):
    ...
    def test_invalid_miner_config(self):
        """
        Test EnhancedBitAxeMonitor behavior with invalid miner configurations.
        
        Verifies that initializing the monitor with an empty configuration raises a ValueError, and that a configuration containing an invalid IP address results in a monitor instance where the miner is present but marked as offline.
        """
        if not ENHANCED_AVAILABLE:
            self.skipTest("Enhanced monitor not available")
    ...
    def test_port_validation(self):
        """
        Test that EnhancedBitAxeMonitor initializes with and correctly sets valid port numbers.
        """
        if not ENHANCED_AVAILABLE:
            self.skipTest("Enhanced monitor not available")
...
class TestUtilities(unittest.TestCase):
    ...
    def test_config_file_loading(self):
        """
        Test that a miner configuration can be correctly loaded from a temporary JSON file and its contents match the expected structure.
        """
...
def create_test_suite():
    """
    Create and return a unittest test suite containing all applicable BitAxe Monitor test classes.
...
def run_tests():
    """
    Run the BitAxe Monitor test suite, print a detailed summary of results, and return whether all tests passed.
