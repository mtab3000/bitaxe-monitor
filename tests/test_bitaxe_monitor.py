...
class TestEnhancedMonitor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        Skips tests if the enhanced monitor is unavailable and initializes a sample miner configuration for use in test cases.
        """
    ...
    def test_miner_config_creation(self):
        """
        Verifies that the MinerConfig dataclass is correctly instantiated with the provided name, IP address, and expected hashrate.
        """
    ...
    def test_variance_tracker(self):
        """
        Tests that the VarianceTracker correctly computes variance statistics after adding multiple data points.
        """
    ...
    def test_bitaxe_api_timeout(self):
        """
        Test that BitAxeAPI returns None when attempting to retrieve system info from an unreachable miner.
        """
    ...
    def test_bitaxe_api_success(self, mock_get):
        ...
        Mocks a successful API response and verifies that the returned data includes expected hashrate and power values.
        """
    ...
    def test_monitor_routes_exist(self):
        """
        Verifies that the main and API routes of the EnhancedBitAxeMonitor Flask app are available and return the expected status codes and response structure.
        """
        monitor = EnhancedBitAxeMonitor(self.test_config, port=8082)
    ...
    def test_offline_miner_handling(self, mock_api):
        """
        Verify that the EnhancedBitAxeMonitor correctly identifies and reports an offline miner when the API returns no data.
        """
    ...
    def test_online_miner_metrics(self, mock_api):
        """
        Test that metrics for an online miner are correctly calculated and reported.
        
        Verifies that when the API returns valid miner data, the monitor reports the miner as online and computes hashrate, power, and efficiency percentage accurately.
        """
        # Mock successful API response
    ...
    def test_fleet_metrics_calculation(self):
        """
        Tests that the EnhancedBitAxeMonitor correctly aggregates fleet-wide metrics such as online miner count, total hashrate (TH), and total power (W) when multiple miners are online.
        """
        monitor = EnhancedBitAxeMonitor(self.test_config, port=8082)
...
class TestClassicMonitor(unittest.TestCase):
    ...
    def test_asic_specs_creation(self):
        """
        Tests retrieval of ASIC specifications for the BM1370 model and verifies that expected specification fields are present.
        """
    ...
    def test_hashrate_history(self):
        """
        Verify that HashrateHistory correctly tracks a fixed number of hashrate readings and computes their average.
        """
...
class TestConfigurationValidation(unittest.TestCase):
    ...
    def test_invalid_miner_config(self):
        ...
        Verifies that creating a monitor with an empty configuration raises an exception, and that a configuration with an invalid IP address results in a miner being marked as offline.
        """
        if not ENHANCED_AVAILABLE:
            self.skipTest("Enhanced monitor not available")
...
def create_test_suite():
    """
    Constructs and returns a unittest test suite including all available monitor and utility tests.
    ...
    Returns:
        unittest.TestSuite: The assembled test suite ready for execution.
    """
...
def run_tests():
    """
    Runs the BitAxe Monitor test suite, prints a summary of results, and returns whether all tests passed.
